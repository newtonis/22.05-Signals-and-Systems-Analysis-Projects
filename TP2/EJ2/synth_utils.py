...
class individual_track:
    ...
    total_time = None
    amp_arr = None
    def __init__(self,ticks_per_beat_,function_,fs):
        self.tempo = None
    ...
    def setFirstTrack(self,track):
        for MetaMessage in track:
            if(MetaMessage.type == "set_tempo"):
                self.bpm = mido.tempo2bpm(MetaMessage.tempo)
                self.tempo = MetaMessage.tempo
    def setTrack(self,track,bpm,tempo):
        self.bpm = bpm
        self.tempo = tempo
        ...
        for note, note_param_arr in self.t_on.items():  # note es la key del dict
            for i in range(len(note_param_arr)):
                note_param_arr[i].delta_t = mido.tick2second(abs(self.t_on[note][i].time - self.t_off[note][i].time), self.ticks_per_beat, self.tempo)
        ...
        segs_per_tick = mido.tick2second(1,self.ticks_per_beat, self.tempo)
